...
incomesmall <- (median(data_all_va$median_income) / 1000)
...
people_renter_occupied <- ((data_all_va$pct_renter_occupied_homes / 100) * data_all_va$population)
...
pct_renter_occupied_homes <- ((sum(people_renter_occupied)) / total_va_population) * 100
...
plot_virginia <- ggplot(data_virginia2, aes(x=county, y=value)) +
  geom_segment(aes(x=county, xend=county, y=0, yend=value), color="#fdb863", lwd = 2) +
...
incomesmall <- (median(data_all_ok$median_income) / 1000)
...
people_renter_occupied <- ((data_all_ok$pct_renter_occupied_homes / 100) * data_all_ok$population)
...
pct_renter_occupied_homes <- ((sum(people_renter_occupied)) / total_ok_population) * 100
...
plot_oklahoma <- ggplot(data_oklahoma2, aes(x=county, y=value)) +
  geom_segment(aes(x=county, xend=county, y=0, yend=value), color="#fdb863") +
  geom_point(color="#EB5F0C", size=5, alpha=0.6) +
  theme_light() +
  coord_flip() +
  ...
  theme(
    ...
    axis.title.y=element_blank(),
    axis.text.x=element_text(size=15),
    axis.text.y=element_text(size=15, colour = c("#fdb863", rep("#232D4B", 6)))
  )
plot_oklahoma
