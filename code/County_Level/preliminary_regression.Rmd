...
final_model <- lm(scale(pct_stayed)~scale(pct_commute_drive)+scale(commute_time_mean)+scale(median_income)+scale(income_ratio)+scale(air_pollution)+scale(pct_white)+scale(segregation_white_non)+scale(population)+scale(pct_rural)+scale(pct_voted), data = selected_10)
...
write_csv(county_embeddedness, "~/ari_social_media/data/final/county_embeddedness.csv")
