...
indicators <- read_csv("~/ari_social_media/data/working/County_Level/county_level_indicators.csv") %>%
  dplyr::select(-c(county, pct_renter_occupied_homes, pct_moved_after_2014,  pct_moved_2010_2014, pct_moved_2000_2009, pct_moved_1990_1999, pct_moved_1980_1989, pct_moved_1979_earlier, pct_movers_within_county, pct_health_ins))
...
missing_indicators <- indicators %>%
  select(-c(pct_lib_program_atten, lib_visit_rate, pct_lib_patrons, lib_hours_per_person, water_violations))%>%
...
indicators_2 <- indicators %>%
  dplyr::select(-c(pct_reg_voted, pct_less_than_hs))
...
#plot_missing(complete_indicators)
```
