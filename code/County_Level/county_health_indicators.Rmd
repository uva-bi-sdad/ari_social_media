...
ACS_social_import <- read_csv("/home/jk9ra/ari_social_media/data/working/County_Level/ACS_17_Social_with_ann.csv")
...
names(ACS_social) <- c("geo_id", "fips", "geo_label", "number_enrolled", "number_enrolled_prek", "number_enrolled_k", "number_enrolled_elementary", "number_enrolled_hs",
...
ACS_housing_import <- read_csv("/home/jk9ra/ari_social_media/data/working/County_Level/ACS_17_Housing_with_ann.csv")
...
names(ACS_housing) <- c("geo_id", "fips", "geo_label", "pct_owner_occupied_homes", "pct_renter_occupied_homes", "pct_moved_after_2014", "pct_moved_2010_2014",
...
ACS_economic_import <- read_csv("/home/jk9ra/ari_social_media/data/working/County_Level/ACS_17_Economic_with_ann.csv")
...
names(ACS_economic) <- c("geo_id", "fips", "geo_label", "pct_unemployed", "pct_commute_drive", "pct_commute_carpool", "pct_commute_public", 
...
ACS_demographic_import <- read_csv("/home/jk9ra/ari_social_media/data/working/County_Level/ACS_17_Demographic_Housing_with_ann.csv")
...
names(ACS_demographic) <- c("geo_id", "fips", "geo_label", "population", "pop_under_5", "pop_5_9", "pop_10_14", "pop_15_19", "pop_20_24", "pct_white",
...
ACS <- ACS_demographic %>%
  right_join(ACS_economic, by=c("geo_id","fips", "geo_label"))%>%
  right_join(ACS_housing, by=c("geo_id","fips", "geo_label"))%>%
  right_join(ACS_social, by=c("geo_id","fips", "geo_label"))
```
...
county_health_rankings <- county_health_ranked_measure_data %>%
...
```{r}
va_library_data <- read_csv("/home/jk9ra/ari_social_media/data/working/County_Level/va_library_data.csv") %>%
  filter(STABR == "VA") %>%
  select("STABR", "CNTY", "POPU_LSA", "HRS_OPEN", "VISITS", "REGBOR", "TOTCIR", "KIDCIRCL", "TOTPRO", "KIDPRO", "YAPRO", "TOTATTEN", "KIDATTEN", "YAATTEN")

names(va_library_data) <- c("state", "county", "lib_lsa_population", "lib_hours_open", "lib_yearly_visits", "lib_registered_users", "lib_circulation", "lib_kids_circulation", "lib_program", "lib_kids_program", "lib_ya_program", "lib_prog_atten", "lib_kids_prog_atten", "lib_ya_prog_atten")


ok_library_data <- read_csv("/home/jk9ra/ari_social_media/data/working/County_Level/va_library_data.csv") %>%
  filter(STABR == "OK") %>%
  select("STABR", "CNTY", "POPU_LSA", "HRS_OPEN", "VISITS", "REGBOR", "TOTCIR", "KIDCIRCL", "TOTPRO", "KIDPRO", "YAPRO", "TOTATTEN", "KIDATTEN", "YAATTEN")

names(ok_library_data) <- c("state", "county", "lib_lsa_population", "lib_hours_open", "lib_yearly_visits", "lib_registered_users", "lib_circulation", "lib_kids_circulation", "lib_program", "lib_kids_program", "lib_ya_program", "lib_prog_atten", "lib_kids_prog_atten", "lib_ya_prog_atten")
```
