...
va_county_health_rankings <- county_health_ranked_measure_data %>%
  ...
  mutate(county = ifelse(county == "Charles City", "Charles City County",
                          ifelse(county == "James City", "James City County",
                                  ifelse(grepl("City", county), as.character(county), paste(as.character(county),"County", sep = " ")))))
...
ok_county_health_rankings <- county_health_ranked_measure_data %>%
  right_join(county_health_additional_measure_data, by=c("fips","state", "county"))%>%
  mutate(county = paste(as.character(county),"County", sep = " "))
...
va_independent_cities <- c("Alexandria", "Bristol", "Buena Vista", "Chesapeake", "Colonial Heights", "Fairfax City", "Falls Church", "Franklin", "Fredericksburg", "Hampton", "Harrisonburg", "Hopewell", "Lexington", "Lynchburg", "Manassas", "Martinsville", "Newport News", "Norfolk", "Petersburg", "Poquoson", "Portsmouth", "Radford", "Richmond", "Roanoke", "Salem", "Staunton", "Suffolk", "Virginia Beach", "Waynesboro", "Williamsburg", "Winchester")

va_kids_count <- va_child_ins %>%
  full_join(va_child_food, by = c("county")) %>%
  full_join(va_child_abuse, by = c("county")) %>%
  full_join(va_foster, by = c("county"))%>%
  filter(county != "Data Type") %>%
  mutate(county = ifelse(county == "Charles City", "Charles City County",
                          ifelse(county == "James City", "James City County",
                                  ifelse(grepl("City", county) | grepl("County", county), as.character(county),
                                         ifelse(county %in% va_independent_cities, paste(as.character(county),"City", sep = " "), paste(as.character(county),"County", sep = " "))))))

rm(va_child_ins, va_child_food, va_child_abuse, va_foster, va_independent_cities)
...
ok_kids_count <- ok_child_ins %>%
  ...
  full_join(ok_foster, by = c("county")) %>%
...
county_health <- read.csv("first_row.csv", header = FALSE, skip=3, col.names=header)%>%
  mutate(county = ifelse(X__3_County == "Charles City", "Charles City County",
                          ifelse(X__3_County == "James City", "James City County",
                                  ifelse(grepl("City", X__3_County), as.character(X__3_County), paste(as.character(X__3_County),"County", sep = " ")))))%>%
