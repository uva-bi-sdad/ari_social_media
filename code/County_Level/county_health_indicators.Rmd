...
va_rankings <- cbind(va_indicators, rank(-rowSums(as.data.frame(cbind(pos_rank, neg_rank)))))%>%
  select(county, `rank(-rowSums(as.data.frame(cbind(pos_rank, neg_rank))))`)
names(va_rankings) <- c("county", "rank")
va_rankings <- va_rankings[order(va_rankings$rank),]
