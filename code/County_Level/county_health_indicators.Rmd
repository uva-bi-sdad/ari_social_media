...
#"TOTCIR", "KIDCIRCL", "KIDPRO", "YAPRO", , "KIDATTEN", "YAATTEN"
...
#, "lib_circulation", "lib_kids_circulation" "lib_kids_program", "lib_ya_program",, "lib_kids_prog_atten", "lib_ya_prog_atten"
...
va_migration <- read.csv("first_row.csv", header = FALSE, skip=3, col.names=header) %>%
  select("State.U.S..Island.Area.Foreign.Region.of.Residence.1.Year.Ago_NA", "County.of.Residence.1.Year.Ago_NA", "County.of.Residence.1.Year.Ago__1_Population.1.Year.and.Over", "X__15_Nonmovers", "X__19_Movers.within.Same.County") %>% #"X__17_Movers.within.United.States", , "X__21_Movers.to.Different.County..Same.State", "X__23_Movers.to.Different.State", "X__25_Movers.to.Puerto.Rico") %>%
...
#"movers_within_us", , "movers_within_state_dif_county", "movers_dif_state", "movers_puerto_rico")
...
rm(va_ACS, va_county_health_rankings, va_kids_count, va_migration, va_library, va_death, ok_ACS, ok_county_health_rankings, ok_kids_count, ok_migration, ok_library, ok_death, va_independent_cities, va_city_county_overlap)
...
```{r}
#want to be higher
pos_indicators <- va_indicators %>%
  select(pct_commute_carpool, pct_commute_public, pct_commute_walk, median_income, pct_health_ins, pct_owner_occupied_homes, pct_attained_hs, pct_bachelors, pct_grad_degree, pct_hs_or_higher, pct_college_or_higher, pct_enrolled_prek, pct_enrolled_elementary_middle, pct_enrolled_hs, pct_less_than_hs, pcp_ratio, social_assoc_rate, median_income_black, median_income_hispanic, pct_stayed, pct_movers_within_county, pct_lib_program_atten, lib_visit_rate, pct_lib_patrons, lib_hours_per_person)

#want to be lower
neg_indicators <- va_indicators %>%
  select(pct_commute_drive, pct_work_home, commute_time_mean, pct_no_health_ins, pct_impoverished, pct_renter_occupied_homes, teen_birth_rate, preventable_hosp_rate, pct_unemployed, pct_children_poverty, income_ratio, air_pollution, severe_housing_problems, pct_food_insecure, segregation_black_white, segregation_white_non, pct_child_no_ins, pct_child_food_insecure, child_abuse_rate, death_rate)




row_ranks <- sapply(va_counties, function(vec) rank(-vec))

pos_rank <- sapply(pos_indicators, function(vec) rank(vec))
neg_rank <- sapply(neg_indicators, function(vec) rank(-vec))

#row_ranks <- as.data.frame(row_ranks)

va_indicators_2 <- cbind(va_indicators, rank(-rowSums(as.data.frame(cbind(pos_rank, neg_rank)))))
