...
ACS <- ACS_demographic %>%
  full_join(ACS_economic, by=c("geo_id","fips", "county", "state"))%>%
  full_join(ACS_housing, by=c("geo_id","fips", "county", "state"))%>%
  full_join(ACS_social, by=c("geo_id","fips", "county", "state"))%>%
  mutate(county = tools::toTitleCase(as.character(county)))%>%
  mutate(pct_enrolled_prek = as.numeric(number_enrolled_prek)/as.numeric(pop_under_5), pct_enrolled_elementary = as.numeric(number_enrolled_elementary)/(as.numeric(pop_5_9) + as.numeric(pop_10_14)), pct_enrolled_hs = as.numeric(number_enrolled_hs)/as.numeric(pop_15_19))%>%
  select(-c(pop_under_5, pop_5_9, pop_10_14, pop_15_19, pop_20_24, number_enrolled, number_enrolled_prek, number_enrolled_k, number_enrolled_elementary, number_enrolled_hs, number_enrolled_college))
...
names(county_health_ranked_measure_data) <- c("fips", "state", "county", "pct_fair_poor_health", "teen_birth_rate", "pcp_ratio", "preventable_hosp_rate", "pct_unemployed", "pct_children_poverty", "income_ratio", "assoc_rate", "air_pollution", "water_violations", "severe_housing_problems", "drive_along_long_commute")
...
va_county_health_rankings <- county_health_ranked_measure_data %>%
  ...
  mutate(county = ifelse(county == "Charles City", "Charles City County",
                          ifelse(county == "James City", "James City County",
                                  ifelse(str_detect(county, "City"), as.character(county), str_c(as.character(county),"County", sep = " ")))))
...
ok_county_health_rankings <- county_health_ranked_measure_data %>%
  right_join(county_health_additional_measure_data, by=c("fips","state", "county"))%>%
  mutate(county = str_c(as.character(county),"County", sep = " "))
...
va_library <- library_data %>%
  ...
  mutate(county = tools::toTitleCase(as.character(county)))%>%
  group_by(county)%>%
  summarise(lib_lsa_population = sum(lib_lsa_population), lib_hours_open = sum(lib_hours_open), lib_yearly_visits = sum(lib_yearly_visits), lib_registered_users = sum(lib_registered_users), lib_circulation = sum(lib_circulation), lib_kids_circulation = sum(lib_kids_circulation), lib_program = sum(lib_program), lib_kids_program = sum(lib_kids_program), lib_ya_program = sum(lib_ya_program), lib_prog_atten = sum(lib_prog_atten), lib_kids_prog_atten= sum(lib_kids_prog_atten), lib_ya_prog_atten = sum(lib_ya_prog_atten))%>%
...
rm(library_data, king_queen_lib, king_william_lib, stafford_lib)
```
...
va_migration <- read.csv("first_row.csv", header = FALSE, skip=3, col.names=header) %>%
  select("State.U.S..Island.Area.Foreign.Region.of.Residence.1.Year.Ago_NA", "County.of.Residence.1.Year.Ago_NA", "County.of.Residence.1.Year.Ago__1_Population.1.Year.and.Over", "X__15_Nonmovers", "X__17_Movers.within.United.States", "X__19_Movers.within.Same.County", "X__21_Movers.to.Different.County..Same.State", "X__23_Movers.to.Different.State", "X__25_Movers.to.Puerto.Rico") %>%
  mutate(County.of.Residence.1.Year.Ago_NA = tools::toTitleCase(as.character(County.of.Residence.1.Year.Ago_NA)))

names(va_migration) <- c("state", "county", "population", "nonmovers", "movers_within_us", "movers_within_county", "movers_within_state_dif_county", "movers_dif_state", "movers_puerto_rico")
...
names(ok_migration) <- c("state", "county", "population", "nonmovers", "movers_within_us", "movers_within_county", "movers_within_state_dif_county", "movers_dif_state", "movers_puerto_rico")
...
# va_foster <- read_excel("/home/jk9ra/ari_social_media/data/working/County_Level/Kids Count/KC_foster-care_virginia_2017.xlsx")
# va_foster <- va_foster %>%
#   mutate(Data = replace(Data, Data ==  "*", NA))%>%
#   filter(LocationType == "County", DataFormat == "percent") %>%
#   dcast(Location ~ `Age group`, value.var = "Data")
# names(va_foster) <- c("county", "pct_foster_1-5", "pct_foster_11-15", "pct_foster_16-18", "pct_foster_6-10", "pct_foster_under_1")

va_kids_count <- va_child_ins %>%
  full_join(va_child_food, by = c("county")) %>%
  full_join(va_child_abuse, by = c("county")) %>%         #full_join(va_foster, by = c("county"))%>%
...
rm(va_child_ins, va_child_food, va_child_abuse) #, va_foster)
...
ok_kids_count <- ok_child_ins %>%
  ...
  full_join(ok_child_abuse, by = c("county")) %>% #full_join(ok_foster, by = c("county")) %>%
...
```{r}
# places_of_worship <- read_csv("/home/jk9ra/ari_social_media/data/working/County_Level/All_Places_of_Worship.csv")
# 
# va_worship <- places_of_worship %>%
#   filter(STATE == "VA") %>%
#   group_by(COUNTY) %>%
#   summarise(places_of_worship = n()) #%>%
#   summarise(sum(places_of_worship))
```
...
names(va_death) <- c("county", "population", "deaths", "fall_deaths", "firearms_deaths", "homocides", "vehicular_deaths", "overdose_deaths", "poison_deaths", "suicide_deaths", "tbi_deaths", "death_rate")
...
names(ok_death) <- c("county", "deaths", "fall_deaths", "firearms_deaths", "homocides", "vehicular_deaths", "poison_deaths", "overdose_deaths", "suicide_deaths", "tbi_deaths", "population", "death_rate")
```
...
va_ACS$fips <- as.numeric(va_ACS$fips)

va_indicators <- va_ACS %>%
  full_join(va_county_health_rankings, by = c("fips", "state", "county"))%>%
...
ok_ACS$fips <- as.numeric(ok_ACS$fips)

ok_indicators <- ok_ACS %>%
  full_join(ok_county_health_rankings, by = c("fips", "state", "county"))%>%
...
rm(va_ACS, va_county_health_rankings, va_kids_count, va_migration, va_library, ok_ACS, ok_county_health_rankings, ok_kids_count, ok_migration, ok_library, va_independent_cities, va_city_county_overlap)
...
###looking at relationship between health rankings and migration data
```{r}
# first_row <- read_excel("/home/jk9ra/ari_social_media/data/working/County_Level/2017_County_Health_Rankings_Virginia_Data-v2.xls", sheet = 2)
# write.csv(first_row,'first_row.csv')
# 
# header <- sapply(read.csv("first_row.csv", nrow=2), paste, collapse="_")
# header <- sapply(read.csv("first_row.csv", header=FALSE, nrow=2), paste, collapse="_")
# 
# county_health <- read.csv("first_row.csv", header = FALSE, skip=3, col.names=header)%>%
#   mutate(county = ifelse(X__3_County == "Charles City", "Charles City County",
#                           ifelse(X__3_County == "James City", "James City County",
#                                   ifelse(str_detect(X__3_County, "City"), as.character(X__3_County), str_c(as.character(X__3_County),"County", sep = " ")))))%>%
#   mutate(state = X__2_State)
# 
# 
# migration_county_rankings <- county_health %>%
#   right_join(va_migration, by=c("county", "state"))
# 
# summary(lm(pct_stayed~X__4_Rank, data = migration_county_rankings))
# mod<- lm(pct_stayed~X__4_Rank, data = migration_county_rankings)
# plot(mod, which=c(1,2))
# 
# rm(county_health, first_row, migration_county_rankings, header, mod)
```